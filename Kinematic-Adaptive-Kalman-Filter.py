...
class KalmanFilter(object):
    ...
    def predict(self, mean, covariance):
        ...
        # Calculate velocity magnitude (vx, vy)
        vel_magnitude = np.linalg.norm(mean[4:6])
        
        # Scale-invariant relative velocity (pixels/frame divided by bounding box height)
        relative_vel = vel_magnitude / max(mean[3], 1e-5)
        
        # Dynamic process noise multiplier: 
        # Base multiplier is 1.0 (static), scales linearly with velocity, capped at 5.0 for stability
        adaptive_multiplier = np.clip(1.0 + relative_vel, 1.0, 5.0)

        std_pos = [
            self._std_weight_position * mean[3] * adaptive_multiplier,
            self._std_weight_position * mean[3] * adaptive_multiplier, 1e-2 * adaptive_multiplier,
            self._std_weight_position * mean[3] * adaptive_multiplier
        ]
        std_vel = [
            self._std_weight_velocity * mean[3] * adaptive_multiplier,
            self._std_weight_velocity * mean[3] * adaptive_multiplier, 1e-5 * adaptive_multiplier,
            self._std_weight_velocity * mean[3] * adaptive_multiplier
        ]
